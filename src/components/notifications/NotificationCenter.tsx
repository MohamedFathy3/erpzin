...
type NotificationItem = {
  id: string;
  type: string;
  data: { type?: string; title?: string; message?: string; category?: string; url?: string };
  read_at: string | null;
  created_at: string;
};
...
const NotificationCenter: React.FC = () => {
  ...
  const { data: notifications = [], isLoading, refetch } = useQuery<NotificationItem[]>({
    ...
    queryFn: async () => (await api.get('/notifications?per_page=50')).data.data?.data ?? [],
  ...
  const text = language === 'ar'
    ? { title: 'الإشعارات', empty: 'لا توجد إشعارات', all: 'تحديد الكل كمقروء', now: 'الآن', min: 'منذ دقائق', hour: 'منذ ساعات', day: 'منذ أيام' }
    : { title: 'Notifications', empty: 'No notifications', all: 'Mark all as read', now: 'Just now', min: 'minutes ago', hour: 'hours ago', day: 'days ago' };
  const relative = (date: string) => {
    const mins = Math.floor((Date.now() - new Date(date).getTime()) / 60000);
    if (mins < 1) return text.now;
    if (mins < 60) return `${mins} ${text.min}`;
    const hours = Math.floor(mins / 60);
    if (hours < 24) return `${hours} ${text.hour}`;
    return `${Math.floor(hours / 24)} ${text.day}`;
  };
  const icon = (type: string) => type === 'success'
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative">
          <Bell size={20} />
          {unreadCount > 0 && <Badge variant="destructive" className="absolute -right-1 -top-1 h-5 w-5 p-0 text-[10px]">{unreadCount > 99 ? '99+' : unreadCount}</Badge>}
        </Button>
      </PopoverTrigger>
