...
const SalesReturns = () => {
  ...
  const {
    ...
    queryKey: ['sales-returns', filters, currentPage, perPage],
    queryFn: async () => {
      try {
        const payload: any = {
          filters: {
  ...
  const meta = returnsResponse?.meta;
  ...
  const fetchReturnDetails = async (returnId: number) => {
    ...
    try {
      ...
      if (response.data.result === 'Success') {
        ...
        // تنسيق العناصر للتأكد من وجود product كstring
        const formattedItems = returnData.items?.map((item: any) => ({
          ...item,
          product: item.product?.name || item.product || 'غير معروف',
                ...
                <Table>
                  ...
                  <TableBody>
                    {returns.map((ret, index) => {
                      const globalIndex = ((meta?.current_page || 1) - 1) * (meta?.per_page || 10) + index + 1;
                      return (
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedReturn && (
            <div className="space-y-4">
              ...
              {/* Refunded Amount */}
              {selectedReturn.refunded_amount && (
                <Card className="border-green-200 bg-green-50">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-green-700">
                        {language === 'ar' ? 'المبلغ المسترد' : 'Refunded Amount'}
                      </span>
                      <span className="font-bold text-green-700">
                        {formatCurrency(selectedReturn.refunded_amount)}
                      </span>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-muted/50">
                          ...
                          <TableHead className="min-w-[120px]">{language === 'ar' ? 'سبب الإرجاع' : 'Reason'}</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                              ...
                              {item.color ? (
                                <Badge variant="outline" className="bg-primary/5 border-primary/20">
                                  <span 
                                    ...
                                    style={{ backgroundColor: typeof item.color === 'string' ? item.color : '#000' }}
                                  />
                                  {item.color}
                                </Badge>
                              ) : (
