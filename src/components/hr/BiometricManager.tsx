...
import BiometricDeviceUsers from '@/components/hr/BiometricDeviceUsers';
  ...
  return <Card className="mt-4">
    ...
    <CardContent>
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-5"><TabsTrigger value="devices"><Wifi size={15} className="me-2" />{ar ? 'الأجهزة' : 'Devices'}</TabsTrigger><TabsTrigger value="users">{ar ? 'أشخاص الجهاز' : 'Device Users'}</TabsTrigger><TabsTrigger value="mapping">{ar ? 'ربط الموظفين' : 'Employee Mapping'}</TabsTrigger><TabsTrigger value="rules"><Settings2 size={15} className="me-2" />{ar ? 'المعادلات' : 'Rules'}</TabsTrigger><TabsTrigger value="reports">{ar ? 'التقارير وExcel' : 'Reports & Excel'}</TabsTrigger></TabsList>
        <TabsContent value="devices" className="space-y-4 pt-4">
          ...
          {devicesLoading ? <p>{ar ? 'جاري التحميل...' : 'Loading...'}</p> : devices.map(device => <div key={device.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div><strong>{device.name}</strong><div className="text-sm text-muted-foreground" dir="ltr">{device.ip_address}:{device.port} / {device.protocol.toUpperCase()}</div>{device.last_error && <div className="text-sm text-destructive">{device.last_error}</div>}</div><div className="flex gap-2"><Button variant="outline" onClick={() => { setSelectedDeviceId(device.id); setActiveTab('users'); }}>{ar ? 'أشخاص الجهاز' : 'Device users'}</Button><Button variant="outline" onClick={() => testDevice.mutate(device.id)}><Wifi size={15} className="me-2" />{ar ? 'اختبار' : 'Test'}</Button><Button onClick={() => syncDevice.mutate(device.id)} disabled={syncDevice.isPending}><RefreshCw size={15} className="me-2" />{ar ? 'سحب البصمات' : 'Sync punches'}</Button></div></div>)}
        </TabsContent>
        <TabsContent value="users"><BiometricDeviceUsers deviceId={selectedDeviceId} employees={employees} /></TabsContent>
