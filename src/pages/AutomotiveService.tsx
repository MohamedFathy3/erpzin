...
const AutomotiveServicePage = () => {
  ...
  const [orderForm, setOrderForm] = useState({ customer_id: '', vehicle_id: '', service_id: '', technician_id: '', quantity: '1', customer_request: '' });
  ...
  const createCustomer = async () => {
    ...
    try { await automotiveApi.createCustomer(customerForm); setCustomerForm({ name: '', phone: '', email: '', address: '' }); setShowCustomerForm(false); toast.success('تمت إضافة العميل'); await load(); }
    catch (error) { toast.error(error instanceof Error ? error.message : 'تعذر إضافة العميل'); }
  };

  const createOrder = async () => {
    const service = services.find((item) => item.id === Number(orderForm.service_id));
    if (!orderForm.customer_id || !orderForm.vehicle_id || !service) return toast.error('اختر العميل والسيارة والخدمة');
    try {
      await automotiveApi.createOrder({ customer_id: Number(orderForm.customer_id), vehicle_id: Number(orderForm.vehicle_id), technician_ids: orderForm.technician_id ? [Number(orderForm.technician_id)] : [], customer_request: orderForm.customer_request || undefined, items: [{ service_id: service.id, description: service.name, quantity: Number(orderForm.quantity || 1), unit_price: Number(service.selling_price), unit_cost: Number(service.estimated_cost || 0) }] });
      setOrderForm({ customer_id: '', vehicle_id: '', service_id: '', technician_id: '', quantity: '1', customer_request: '' }); setShowOrderForm(false); toast.success('تم إنشاء أمر الخدمة وربط الخدمة بالعميل والسيارة'); await load();
    } catch (error) { toast.error(error instanceof Error ? error.message : 'تعذر إنشاء أمر الخدمة'); }
  };
  ...
  const createCustomerAccount = async () => {
    ...
    try {
      ...
      setCreatedCredentials((current) => ({ ...current, [portalForm.customer_id]: { email: portalForm.email, password: portalForm.password } }));
      setPortalForm({ customer_id: 0, email: '', password: '' });
      toast.success('تم إنشاء دخول العميل للبوابة');
    } catch (error) { toast.error(error instanceof Error ? error.message : 'تعذر إنشاء حساب العميل'); }
  };
  ...
  return (
    <MainLayout activeItem="automotive"><div className="min-h-full space-y-6 p-6" dir="rtl">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-sm font-medium text-primary">Automotive Service Center</p><h1 className="text-3xl font-bold tracking-tight">خدمة السيارات</h1><p className="mt-1 text-muted-foreground">مركز موحد للعملاء والسيارات والخدمات وأوامر الصيانة.</p></div>
        <div className="flex flex-wrap gap-2"><Button onClick={() => setShowOrderForm((value) => !value)}><Plus className="ml-2 h-4 w-4" /> بيع خدمة / أمر جديد</Button><Button variant="outline" onClick={() => setShowCustomerForm((value) => !value)}><Users className="ml-2 h-4 w-4" /> عميل جديد</Button><Button variant="secondary" onClick={() => navigate('/pos')}><DollarSign className="ml-2 h-4 w-4" /> POS</Button><Button variant="secondary" onClick={() => navigate('/POSRetrun')}><RefreshCw className="ml-2 h-4 w-4" /> مرتجعات POS</Button><Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className="ml-2 h-4 w-4" /> تحديث</Button></div>
      </div>
      {showCustomerForm && <Card className="border-primary/20 bg-primary/[0.03]"><CardHeader><CardTitle>إضافة عميل من خدمة السيارات</CardTitle></CardHeader><CardContent><div className="grid gap-3 md:grid-cols-4"><div><Label>الاسم *</Label><Input value={customerForm.name} onChange={(e) => setCustomerForm({ ...customerForm, name: e.target.value })} /></div><div><Label>الهاتف</Label><Input value={customerForm.phone} onChange={(e) => setCustomerForm({ ...customerForm, phone: e.target.value })} /></div><div><Label>البريد الإلكتروني</Label><Input type="email" value={customerForm.email} onChange={(e) => setCustomerForm({ ...customerForm, email: e.target.value })} /></div><div><Label>العنوان</Label><Input value={customerForm.address} onChange={(e) => setCustomerForm({ ...customerForm, address: e.target.value })} /></div></div><div className="mt-4 flex justify-end gap-2"><Button variant="ghost" onClick={() => setShowCustomerForm(false)}>إلغاء</Button><Button onClick={() => void createCustomer()}>حفظ العميل</Button></div></CardContent></Card>}
      {showOrderForm && <Card className="border-emerald-500/20 bg-emerald-500/[0.03]"><CardHeader><CardTitle>بيع خدمة وإنشاء أمر صيانة</CardTitle></CardHeader><CardContent><div className="grid gap-3 md:grid-cols-4"><div><Label>العميل *</Label><select className="h-10 w-full rounded-md border bg-background px-3" value={orderForm.customer_id} onChange={(e) => setOrderForm({ ...orderForm, customer_id: e.target.value, vehicle_id: '' })}><option value="">اختر العميل</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select></div><div><Label>السيارة *</Label><select className="h-10 w-full rounded-md border bg-background px-3" value={orderForm.vehicle_id} onChange={(e) => setOrderForm({ ...orderForm, vehicle_id: e.target.value })}><option value="">اختر سيارة العميل</option>{vehicles.filter((vehicle) => !orderForm.customer_id || vehicle.customer_id === Number(orderForm.customer_id)).map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.make} {vehicle.model} · {vehicle.plate_number || 'بدون لوحة'}</option>)}</select></div><div><Label>الخدمة *</Label><select className="h-10 w-full rounded-md border bg-background px-3" value={orderForm.service_id} onChange={(e) => setOrderForm({ ...orderForm, service_id: e.target.value })}><option value="">اختر الخدمة</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name} · {Number(service.selling_price).toLocaleString()}</option>)}</select></div><div><Label>الفني المسؤول</Label><select className="h-10 w-full rounded-md border bg-background px-3" value={orderForm.technician_id} onChange={(e) => setOrderForm({ ...orderForm, technician_id: e.target.value })}><option value="">اختر الفني</option>{technicians.filter((technician) => { const role = typeof technician.role === "string" ? technician.role : technician.role?.name; return role?.toLowerCase() === "technician"; }).map((technician) => <option key={technician.id} value={technician.id}>{technician.name}</option>)}</select></div><div><Label>الكمية</Label><Input type="number" min="1" value={orderForm.quantity} onChange={(e) => setOrderForm({ ...orderForm, quantity: e.target.value })} /></div></div><div className="mt-3"><Label>طلب العميل</Label><Input value={orderForm.customer_request} onChange={(e) => setOrderForm({ ...orderForm, customer_request: e.target.value })} placeholder="وصف العطل أو المطلوب" /></div><div className="mt-4 flex justify-end gap-2"><Button variant="ghost" onClick={() => setShowOrderForm(false)}>إلغاء</Button><Button onClick={() => void createOrder()}>إنشاء أمر وربط الخدمة</Button></div></CardContent></Card>}
