...
const AutomotiveServicePage = () => {
  ...
  const [orderForm, setOrderForm] = useState({ customer_id: '', vehicle_id: '', service_id: '', quantity: '1', customer_request: '' });
  ...
  const createOrder = async () => {
    ...
    try {
      await automotiveApi.createOrder({ customer_id: Number(orderForm.customer_id), vehicle_id: Number(orderForm.vehicle_id), customer_request: orderForm.customer_request || undefined, items: [{ service_id: service.id, description: service.name, quantity: Number(orderForm.quantity || 1), unit_price: Number(service.selling_price), unit_cost: Number(service.estimated_cost || 0) }] });
      setOrderForm({ customer_id: '', vehicle_id: '', service_id: '', quantity: '1', customer_request: '' }); setShowOrderForm(false); toast.success('تم إنشاء أمر الخدمة وربط الخدمة بالعميل والسيارة'); await load();
