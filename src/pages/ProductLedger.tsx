...
interface LedgerRow { id: number; date?: string; reference?: string | number; quantity?: number; quantity_delta?: number; unit_price?: number; total?: number; total_cost?: number; type?: string; note?: string; variant?: { product_unit_id?: number | null; size?: string | null; color_id?: number | null; color?: string | null }; customer?: { name?: string; name_ar?: string; phone?: string }; supplier?: { name?: string; name_ar?: string; phone?: string }; warehouse?: { name?: string }; branch?: { name?: string }; }
...
function LedgerTable({ rows, kind, isArabic, label, formatDate }: { rows: LedgerRow[]; kind: 'sales' | 'purchases' | 'movements'; isArabic: boolean; label: (row: LedgerRow, field: 'customer' | 'supplier') => string; formatDate: (date?: string) => string }) {
  ...
  const movementLabels: Record<string, string> = isArabic ? {
    branch_transfer_out: 'تحويل صادر إلى فرع', branch_transfer_in: 'تحويل وارد من فرع',
    transfer_out: 'تحويل صادر', transfer_in: 'تحويل وارد', purchase: 'شراء', sale: 'بيع',
  } : {
    branch_transfer_out: 'Branch transfer out', branch_transfer_in: 'Branch transfer in',
    transfer_out: 'Transfer out', transfer_in: 'Transfer in', purchase: 'Purchase', sale: 'Sale',
  };
  return <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead><tr className="border-b text-start text-muted-foreground"><th className="p-3 text-start">{isArabic ? 'التاريخ' : 'Date'}</th><th className="p-3 text-start">{isArabic ? 'المرجع' : 'Reference'}</th><th className="p-3 text-start">{kind === 'sales' ? (isArabic ? 'العميل' : 'Customer') : kind === 'purchases' ? (isArabic ? 'المورد' : 'Supplier') : (isArabic ? 'نوع الحركة' : 'Movement')}</th><th className="p-3 text-start">{isArabic ? 'اللون / المقاس' : 'Color / Size'}</th><th className="p-3 text-start">{isArabic ? 'الكمية' : 'Quantity'}</th><th className="p-3 text-start">{isArabic ? 'الإجمالي' : 'Total'}</th><th className="p-3 text-start">{isArabic ? 'المخزن / الفرع' : 'Warehouse / Branch'}</th></tr></thead><tbody>{rows.map(row => <tr key={`${kind}-${row.id}`} className="border-b last:border-0"><td className="p-3">{formatDate(row.date)}</td><td className="p-3 font-medium">{row.reference || '-'}</td><td className="p-3">{kind === 'sales' ? label(row, 'customer') : kind === 'purchases' ? label(row, 'supplier') : <Badge variant="outline">{movementLabels[row.type || ''] || row.type || row.note || '-'}</Badge>}</td><td className="p-3"><div className="flex flex-wrap gap-1">{row.variant?.color && <Badge variant="outline">{isArabic ? 'لون: ' : 'Color: '}{row.variant.color}</Badge>}{row.variant?.size && <Badge variant="outline">{isArabic ? 'مقاس: ' : 'Size: '}{row.variant.size}</Badge>}{!row.variant?.color && !row.variant?.size && '-'}</div></td><td className="p-3">{row.quantity_delta !== undefined ? Math.abs(row.quantity_delta) : row.quantity ?? 0}</td><td className="p-3">{money(row.total ?? row.total_cost)}</td><td className="p-3"><div>{row.warehouse?.name || '-'}</div>{row.branch?.name && <div className="text-xs text-muted-foreground">{row.branch.name}</div>}</td></tr>)}</tbody></table></div>;
}
