  ...
  import { useCurrencyTax } from '@/hooks/useCurrencyTax'; // ✅ إضافة import الضريبة
...
import api from '@/lib/api';
  ...
  const POS: React.FC = () => {
    ...
    const { taxRates } = useCurrencyTax(); // ✅ جلب الضرائب
...
const branchData = useMemo(() => {
  ...
  return {
    ...
    phone: branchDetails?.phone || null, // ✅ من الـ API
    address: branchDetails?.address || null, // ✅ من الـ API
    addressAr: branchDetails?.address_ar || branchDetails?.address || null // ✅ من الـ API
  };
    ...
    const holdOrder = () => {
      ...
      const tax = (subtotal * taxRate) / 100; // ✅ استخدام taxRate الديناميكي
    ...
    const calculateTax = () => (calculateSubtotal() * taxRate) / 100; // ✅ نسبة مئوية
      ...
      if (!navigator.onLine || isOffline) {
        try {
          const orderId = await saveOrderOffline({
            ...
            delivery_id: selectedDelivery?.id, // ✅ هنا بيتبعت
