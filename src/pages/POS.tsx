        ...
        <Dialog open={showInvoiceTransfers} onOpenChange={setShowInvoiceTransfers}>
          <DialogContent className="max-w-2xl">
            ...
            <div className="rounded-lg border p-3">
              ...
              {transferRequests.filter((request: any) => request.status === 'pending').length === 0 ? <p className="text-sm text-muted-foreground">{language === 'ar' ? 'لا توجد طلبات معلقة' : 'No pending requests'}</p> : transferRequests.filter((request: any) => request.status === 'pending').map((request: any) => <div key={`admin-${request.id}`} className="flex flex-wrap items-center justify-between gap-2 border-t py-2 text-sm"><span>#{request.invoice_id} · {request.from_employee?.name || '-'} → {request.to_employee?.name || '-'}</span><span className="flex gap-2"><Button size="sm" onClick={() => handleInvoiceTransferDecision(request.id, 'approve')}>{language === 'ar' ? 'موافقة' : 'Approve'}</Button><Button size="sm" variant="outline" onClick={() => handleInvoiceTransferDecision(request.id, 'reject')}>{language === 'ar' ? 'رفض' : 'Reject'}</Button></span></div>)}
            </div>
