...
type Section = { intent?: string; rows?: Row[]; clarification_needed?: string | null };
type Message = { role: 'user' | 'assistant' | 'error'; content: string; data?: Row[]; sections?: Section[] };
...
export default function AIAssistant() {
  ...
  const send = async (value = input) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: data.answer, data: data.data || [], sections: data.sections || [] }]);
    } catch (error: any) {
  ...
  const renderRows = (rows?: Row[]) => rows?.length ? <div className="mt-3 max-w-full overflow-x-auto rounded-lg border bg-background text-foreground"><table className="w-full text-xs"><thead className="bg-muted"><tr>{Object.keys(rows[0]).map(k => <th key={k} className="whitespace-nowrap px-3 py-2 text-start font-semibold">{k}</th>)}</tr></thead><tbody>{rows.slice(0, 100).map((row, ri) => <tr key={ri} className="border-t">{Object.keys(rows[0]).map(k => <td key={k} className="whitespace-nowrap px-3 py-2">{String(row[k] ?? '—')}</td>)}</tr>)}</tbody></table></div> : null;
