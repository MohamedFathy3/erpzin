...
import { Bell, CalendarClock, CheckCircle2, Clock3, Plus, RefreshCw, Trash2 } from 'lucide-react';
...
export default function Tasks() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const response = await api.post('/tasks', { title, description: description || undefined, due_date: dueDate || undefined, priority });
      if (remindAt) await api.post(`/tasks/${response.data.data.id}/reminders`, { remind_at: remindAt, channel });
      return response;
    },
    onSuccess: () => { toast.success('تم إنشاء المهمة'); setTitle(''); setDescription(''); setDueDate(''); setRemindAt(''); queryClient.invalidateQueries({ queryKey: ['tasks'] }); },
  ...
  return <MainLayout activeItem="tasks"><div className="space-y-6" dir="rtl">
    ...
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><Plus className="h-5 w-5" />مهمة جديدة</CardTitle></CardHeader><CardContent className="space-y-4"><div><Label>العنوان</Label><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="مثال: متابعة عرض السعر" /></div><div><Label>الوصف</Label><Textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} /></div><div><Label>موعد الاستحقاق</Label><Input type="datetime-local" value={dueDate} onChange={e => setDueDate(e.target.value)} dir="ltr" /></div><div><Label>الأولوية</Label><Select value={priority} onValueChange={setPriority}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(priorityLabels).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select></div><div className="border-t pt-4"><Label>تذكير اختياري</Label><Input className="mt-2" type="datetime-local" value={remindAt} onChange={e => setRemindAt(e.target.value)} dir="ltr" />{remindAt && <Select value={channel} onValueChange={setChannel}><SelectTrigger className="mt-2"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="in-app">داخل النظام</SelectItem><SelectItem value="email">البريد الإلكتروني</SelectItem><SelectItem value="whatsapp">WhatsApp</SelectItem></SelectContent></Select>}</div><Button className="w-full" disabled={!title || createMutation.isPending} onClick={() => createMutation.mutate()}><Plus className="me-2 h-4 w-4" />{createMutation.isPending ? 'جاري الحفظ...' : 'حفظ المهمة'}</Button></CardContent></Card>
