...
export default function AccessControl() {
  ...
  const visibleRoles = useMemo(() => roles.filter((role) => {
    const matchesSearch = role.name.toLowerCase().includes(roleSearch.toLowerCase()) || tenantLabel(role).toLowerCase().includes(roleSearch.toLowerCase());
    ...
    <div className="min-h-full space-y-6 bg-muted/20 p-4 md:p-6" dir="rtl">
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
        ...
        <Card className="min-h-[560px] shadow-sm">
          <CardHeader className="border-b bg-background"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><CardTitle className="text-xl">{selected ? `تعديل صلاحيات: ${selected.name}` : 'اختر دوراً للبدء'}</CardTitle>{selected && <p className="mt-1 text-sm text-muted-foreground">Tenant: <span className="font-semibold text-foreground">{tenantLabel(selected)}</span></p>}</div>{selected && <Badge className="w-fit gap-1"><Check size={14} /> {checked.length} صلاحية مفعلة</Badge>}</div></CardHeader>
          <CardContent className="space-y-5 p-5">
            ...
            {selected && <>
              <div className="grid gap-4 rounded-xl border bg-muted/20 p-4 md:grid-cols-[1fr_auto] md:items-end"><div><label className="text-sm font-medium">اسم الدور</label><Input className="mt-2 bg-background" value={name} onChange={(event) => setName(event.target.value)} /></div><div className="rounded-lg bg-background px-4 py-2 text-sm ring-1 ring-border"><span className="text-muted-foreground">Tenant: </span><strong>{tenantLabel(selected)}</strong></div></div>
